    ...
    for (final dir in imageDir) {
      ...
      for (FileSystemEntity entity in fileList) {
        ...
        switch(lookupMimeType(entity.path)) {
          ...
          case 'image/gif':
          case 'image/webp':
            animationList[path.basenameWithoutExtension(entity.path)] = entity.path;
            break;
          default:
            break;
        }
...
'''
    ...
    imageList.forEach((key, value) {
      outputBuffer.writeln('  AssetGenImage get ${_formatImgKeyName(key)} => const AssetGenImage("$value");');
    });
    outputBuffer.writeln("}");
...
'''
  ...
  String _formatImgKeyName(String fileName) {
    ...
    final wordsTmp = words.map((w) => '${w.substring(0, 1).toUpperCase()}${w.substring(1).toLowerCase()}').toList();
